...
plot(steadystate_t{i},Istims_AC{i}(steadystate_t{i}),'LineWidth',1.2)
hold on;
end
hold off;
...
subplot(3,2,2);
for i=1:length(steadystate_t)
plot(steadystate_t{i},steadystate_y{i}(:,1),'LineWidth',1.2)
hold on;
end
hold off;
ax = gca; % Get the current axes object
...
plot(forward_ts{i},forward_ys{i}(:,1),'LineWidth',1.2)
hold on;
end
hold off;
ax = gca; % Get the current axes object
