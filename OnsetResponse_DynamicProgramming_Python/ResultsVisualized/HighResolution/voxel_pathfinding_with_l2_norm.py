...
# 
voxel_dim = np.array([0.01, 0.005, 0.005, 0.005])
...
transitions_file = f'transitions-overnight-{"-".join([str(x) for x in n_voxels])}'
path_grid_file = f'path-grid-l2-for-{transitions_file}.npy'
...
def cost_from_index(ai):
    """cost must be proportional to L^2 norm of the stimulus"""
    return discrete_amplitudes[ai]**2
    ...
    for i in range(max_iter):
        ...
        for index in all_state_indices():
            ...
            if index not in quick_lookup:
                continue
            for (start_state_index,_,ai) in quick_lookup[index]:
                new_cost = cost + cost_from_index(ai)
                if new_cost < path_grid[tuple(start_state_index)]['cost']:
                    ...
                    if changes_value is None or new_cost < changes_value[2]:
                        changes[tuple(start_state_index)] = (index, ai, new_cost)
